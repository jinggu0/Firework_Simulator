...
# ---------------------------------------------------------------------------
# V-21 asset checksum integrity
# ---------------------------------------------------------------------------


# Provenance path to the asset whose checksum it declares. The record schema is
# fixed by the fidelity contract and has no filename field, so the mapping is
# explicit here rather than parsed out of free-text notes.
CHECKSUMMED_ASSETS: dict[str, str] = {
    "environment.weather": "yeouido_2024-10-05_environment.json",
    "scene.osm": "yeouido_scene.npz",
    "scene.site_details": "yeouido_official_facilities.json",
}


def asset_checksum_integrity(scenario: Scenario) -> MetricResult:
    """Verify every declared checksum against the file it stands for."""
    ...
    for path, filename in CHECKSUMMED_ASSETS.items():
        record = scenario.provenance.record_for(path)
        ...
        if not asset.exists():
            ...
            continue
        actual = file_checksum(asset)
        checked[path] = filename
        if actual != record.checksum:
            mismatched.append(
                f"{path} ({filename}): declared {record.checksum[:19]}..., "
                f"actual {actual[:19]}..."
            )

    passed = not mismatched and not missing_files and bool(checked)
    return MetricResult(
        spec=catalogue.V21,
        status=MetricStatus.PASS if passed else MetricStatus.FAIL,
        message=(
            f"{len(checked)} declared checksum(s) verified"
            + (f"; {len(mismatched)} mismatched" if mismatched else "")
            + (f"; {len(missing_files)} file(s) absent" if missing_files else "")
        ),
        residuals={
            "checksums_verified": float(len(checked)),
            "checksums_mismatched": float(len(mismatched)),
        },
        detail={
            "verified": checked,
            "mismatched": mismatched,
            "missing_files": missing_files,
            "records_without_a_checksum": undeclared,
            "note": (
                "Derived assets are regenerated by tools in this repository, "
                "so a checksum drifts as a normal consequence of rebuilding "
                "one. Re-run the importer or builder and update the record."
            ),
        },
    )
